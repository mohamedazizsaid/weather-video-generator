...
def forgot_password(request):
    if request.method == "POST":
        ...
        if user:
            # Appel à l'API Brevo pour tester la connexion
            headers = {
                "accept": "application/json",
                "api-key": "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"
            }

            response = requests.get("https://api.brevo.com/v3/account", headers=headers)
            print(response.status_code)  # Affiche le code de statut pour le test
            print(response.json())  # Affiche la réponse de l'API

            # Si l'appel API est réussi, on procède à l'envoi de l'email
